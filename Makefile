...
RUN_DEPENDS=	python>=3.6_3,2:lang/python
BUILD_DEPENDS=	go>=1.11.5,1:lang/go \
								cmake>=3.12.0:devel/cmake \
								python>=3.6_3,2:lang/python
...
.endfor
	...
	${INSTALL_DATA} ${GO_WRKSRC}/cmd/agent/dist/datadog.yaml ${STAGEDIR}${ETCDIR}/datadog.yaml.example
	${SED} -ie 's/^# confd_path\: ""/confd_path: "\/usr\/local\/etc\/datadog\/conf.d"/g' ${STAGEDIR}${ETCDIR}/datadog.yaml.example
